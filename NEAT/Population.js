...
export class Population {
  ...
  constructor(size, playerContructor) {
    ...
    this.genPlayers = []; //new ArrayList<BasePlayer>();
    ...
    for (var i = 0; i < size; i++) {
      this.players.push(playerContructor());
    ...
    this.firstLivingPlayer = this.players[0];
  }
  ...
  updateAlive(context) {
    for (let i = 0; i < this.players.length; i++) {
      if (!this.players[i].dead) {
        ...
        if (!this.showNothing && (!this.showBest || i === 0)) {
          this.players[i].show(context);
        }
        if (this.players[i].score > this.globalBestScore) {
          this.globalBestScore = this.players[i].score;
        }
        this.firstLivingPlayer = this.players[i];
      }
    }
  ...
  naturalSelection() {
    ...
    this.players = [];
    this.players = [...children]; //set the children as the current this.playersulation
    this.gen += 1;
  ...
  playerInBatch(player) {
    for (let i = this.batchNo * this.worldsPerBatch;
